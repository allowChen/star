...
<script>
  ...
  export default {
    ...
    computed: {
      health() {
        let str = this.$store.state.nextWeek.health
        if (str) {
          return str.substring(3,);
        }else {
          return this.$store.state.nextWeek.health
        }
      },
      job() {
        let str = this.$store.state.nextWeek.job
        if (str) {
          return str.substring(3,);
        }else {
          return this.$store.state.nextWeek.job
        }
      },
      love() {
        let str = this.$store.state.nextWeek.love
        if (str) {
          return str.substring(3,);
        }else {
          return this.$store.state.nextWeek.love
        }
      },
      money() {
        let str = this.$store.state.nextWeek.money
        if (str) {
          return str.substring(3,);
        }else {
          return this.$store.state.nextWeek.money
        }
      },
      work() {
        let str = this.$store.state.nextWeek.work
        if (str) {
          return str.substring(3,);
        }else {
          return this.$store.state.nextWeek.work
        }
      }
    },
    components: {
